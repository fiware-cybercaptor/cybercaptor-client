...
function transformGraphTopoDRA(data, alert){
    ...
    if(alert != undefined){
        ...
        for(var i=0; i < alert.dynamic_remediations.length; ++i){
            console.log(alert.dynamic_remediations[i]);
            for(var y=0; y < alert.dynamic_remediations[i].length; ++y){
                tmpRemed = alert.dynamic_remediations[i];
                remediations.push(tmpRemed);
            }
        }

        console.log(tmpRemed);
        ...
        for(var y=0; y < attackGraphDatas.links.length; ++y){
            for(var v=0; v < alert.CVEs.length; ++v){
                ...
                if(attackGraphDatas.links[y].label == alert.CVEs[v].CVE){
                    console.log(attackGraphDatas.links[y]);
                    console.log(alert.CVEs[v].CVE);
    ...
    function tst(tab, color){
        for(var z=0; z < tab.length; ++z){
            ...
            console.log(attackGraphDatas.nodes[i].IP + " ? " + tab[z]+ " = " + result);
            if(result == 0){
        ...
        for(var z=0; z < tab.length; ++z){
                var result = (attackGraphDatas.nodes[i].Name).indexOf(tab[z]);
                        console.log(attackGraphDatas.nodes[i].Name + " ? " + tab[z]+ " = " + result);
                        if(result == 0){
                            attackGraphDatas.nodes[i].Visu = color;
                            attackGraphDatas.nodes[i].Size = size;
                            attackGraphDatas.nodes[i].Opacity = opacity;
